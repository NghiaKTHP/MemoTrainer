"""
Convert DinoV3ConvNextMask2Former .onnx -> .trt (TensorRT engine) voi
DYNAMIC BATCH profile.

- Quet WEIGHTS_DIR tim tat ca file .onnx (bo qua *_original.onnx).
- Voi moi file: build TRT engine voi optimization profile
  min=[MIN_BATCH,3,H,W], opt=[OPT_BATCH,3,H,W], max=[MAX_BATCH,3,H,W].
- Chi hoat dong khi .onnx da export voi dynamic_axes cho batch (dim 0).
- H/W FIX bang IMAGE_SIZE (M2F pixel decoder co int(shape) bake vao trace).

Yeu cau: pip install tensorrt (matched voi CUDA runtime dang dung).

Luu y M2F op requirements:
- TRT >= 8.6 de support native GridSample (dung boi pure-Python MSDeformAttn).
- Neu TRT < 8.6, can custom plugin.
"""
...
IMAGE_SIZE   = 640     # phai match voi luc export ONNX (chia het cho 32)

# Batch profile (batch=0-axis dynamic).
MIN_BATCH    = 1
OPT_BATCH    = 4       # batch dung nhat luc inference
MAX_BATCH    = 8

INPUT_NAME   = "input"

# FP16 giup nhanh + tiet kiem VRAM. M2F chay fp16 kha on dinh.
USE_FP16     = True

# Workspace GB (M2F transformer decoder can nhieu workspace hon UperNet).
WORKSPACE_GB = 8
